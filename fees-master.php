                        ...
                        <div class="custom-datatable-filter table-responsive">
                            <table class="table datatable">
                                ...
                                <tbody>
                                    ...
                                    if ($mysqli) {
                                        $stmt = $mysqli->prepare("SELECT feescollection.ID, addstudent.ID, addstudent.fnameOfStudent, addstudent.lnameOfStudent, addstudent.class, addstudent.rollNumber, addstudent.feesGroup, feescollection.courseFee, feescollection.discount, feescollection.remarks,  feescollection.pendingAmount, feescollection.finalPendingAmount, feescollection.AmountPaid FROM `feescollection` INNER JOIN addstudent ON addstudent.ID = feescollection.StudentID ORDER BY feescollection.ID DESC;");
                                        $stmt->execute();
                                        $stmt->store_result();
                                        $stmt->bind_result($ID, $studentID, $fnameOfStudent, $lnameOfStudent, $class, $rollNumber, $feesGroup, $courseFee, $discount, $remark, $pendingAmount, $finalPendingAmount, $AmountPaid);
                                        $sn = 1;
                                        while ($stmt->fetch()) {
                                            echo "<tr>";
                                            echo "<td>";
                                            echo "<div class='form-check form-check-md'>";
                                            echo "<input class='form-check-input' type='checkbox' id='checkbox-1'>";
                                            echo "</div>";
                                            echo "</td>";
                                            echo "<td>" . $sn . "</td>";
                                            echo "<td>" . $ID . "</td>";
                                            echo "<td>" . $fnameOfStudent . " " . $lnameOfStudent . "</td>";
                                            echo "<td>" . $class . "</td>";
